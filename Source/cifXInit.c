...
***************************************************************************************
  ...
  $Id: cifXInit.c 13262 2019-10-16 11:06:11Z Robert $:
...
extern int      IsNetX4x00                  ( PDEVICEINSTANCE ptDevInstance,
                                              int      fGlobalRegisterBlock);
extern int      IsNetX90                    ( PDEVICEINSTANCE ptDevInstance,
                                              int      fGlobalRegisterBlock);
...
static int32_t cifXHardwareReset(PDEVICEINSTANCE ptDevInstance)
{
  ...
  if( (eCHIP_TYPE_NETX51 == ptDevInstance->eChipType) ||
      (eCHIP_TYPE_NETX52 == ptDevInstance->eChipType)  )
  {
  ...
  if(ptDevInstance->fPCICard)
  {
    /* netX4000 based PCI devices do not need PCIConfiguration Space reapplied */
    if( ( eCHIP_TYPE_NETX4000 != ptDevInstance->eChipType) &&
  ...
  for(ulIdx = 0; (ulIdx < NET_BOOTLOADER_STARTUP_CYCLES) && (lRet != CIFX_NO_ERROR); ++ulIdx)
  {
    ...
    } else
    {
      switch (ptDevInstance->eChipType)
      {
        ...
        default:
          /* Unknown chip type, return with an error in lRet */
        break;
      }
    }
  }
...
*   \return CIFX_NO_ERROR on success                                         */
/*****************************************************************************/
void cifXDetectChipType(PDEVICEINSTANCE ptDevInstance, int fGlobalRegisterBlock)
{
  uint32_t ulCookie;
  ...
  /*-----------------------------*/
  } else if(IsNetX4x00(ptDevInstance, fGlobalRegisterBlock))
  {
  ...
  /* Check for netX90            */
  /*-----------------------------*/
  } else if(IsNetX90(ptDevInstance, fGlobalRegisterBlock))
  {
    /* eChipType already set */
...
static int32_t cifXRunBootloader(PDEVICEINSTANCE ptDevInstance)
{
  ...
  /* Detect Chip type */
  cifXDetectChipType( ptDevInstance, 0);
...
static int32_t cifXStartDevice(PDEVICEINSTANCE ptDevInstance)
{
  ...
  {
    /* If firmware is running, expect netX90/netX4000 to setup DPM register block */
    if((NETX_DPM_MEMORY_SIZE == ptDevInstance->ulDPMSize) &&
       ( (eCIFX_DEVICE_FLASH_BASED == ptDevInstance->eDeviceType) ||
         (eCIFX_DEVICE_DONT_TOUCH  == ptDevInstance->eDeviceType)) )
    {
      /* Perform check on global register block (end of DPM memory) */
      cifXDetectChipType(ptDevInstance, 1);
    }
    else
    {
      /* Perform check at DPM start */
      cifXDetectChipType(ptDevInstance, 0);
    }
    ...
    switch(ptDevInstance->eDeviceType)
    {
      ...
      case eCIFX_DEVICE_FLASH_BASED:
        ...
        /*-----------------------------------*/
        if( (eCHIP_TYPE_NETX90   == ptDevInstance->eChipType) ||
