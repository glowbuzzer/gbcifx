...
***************************************************************************************
  ...
  $Id: cifXHWFunctions.c 13639 2020-08-18 07:01:11Z LuisContreras $:
...
*   \return CIFX_NO_ERROR on success                                         */
/*****************************************************************************/
static int32_t DEV_Reset_Execute(PDEVICEINSTANCE ptDevInstance, uint8_t bHostFlagsChange )
{
...
int32_t DEV_DoSystemStart(PCHANNELINSTANCE ptChannel, uint32_t ulTimeout, uint32_t ulParam)
{
  ...
  }else
  {
    ...
    uint32_t ulSystemControl = HIL_SYS_CONTROL_RESET_MODE_COLDSTART | (ulParam & HIL_SYS_CONTROL_RESET_PARAM_MASK);
         ...
         HIL_SYS_STATUS_APP  == (HIL_SYS_STATUS_APP  & ulSystemStatus) )
    {
       ...
       * will be executed. The application on the APP CPU has to
       * request the reset using the MCP registers. */
      lRet = CIFX_NO_ERROR;
    } else
    {
      /* Prepare reset */
      ...
      /* Perform the Reset */
      lRet = DEV_Reset_Execute(ptDevInstance, HSF_RESET );
...
int32_t DEV_DoSystemBootstart(PCHANNELINSTANCE ptChannel, uint32_t ulTimeout, uint32_t ulParam)
{
  ...
  } else
  {
    ...
    uint32_t ulSystemControl = HIL_SYS_CONTROL_RESET_MODE_BOOTSTART | (ulParam & HIL_SYS_CONTROL_RESET_PARAM_MASK);
    HWIF_WRITE32(ptDevInstance, ptSysChannel->tSystemControl.ulSystemControl, HOST_TO_LE32((ulSystemControl)));
    ...
    } else
    {
      ...
      lRet = DEV_Reset_Execute(ptDevInstance, (uint8_t)(HSF_RESET | HSF_BOOTSTART));
...
int32_t DEV_DoUpdateStart(PCHANNELINSTANCE ptChannel, uint32_t ulTimeout, uint32_t ulParam)
{
  ...
  }else
  {
    ...
    /* Write the reset mode to DPM */
    uint32_t ulSystemControl = HIL_SYS_CONTROL_RESET_MODE_UPDATESTART | (ulParam & HIL_SYS_CONTROL_RESET_PARAM_MASK);
    HWIF_WRITE32(ptDevInstance, ptSysChannel->tSystemControl.ulSystemControl, HOST_TO_LE32((ulSystemControl)));
...
{
  uint8_t bData = 0;
  (void)ptDev->pfnHwIfRead(ptDev, pvSrc, &bData, 1);
  return bData;
}
...
{
  uint16_t usData = 0;
  (void)ptDev->pfnHwIfRead(ptDev, pvSrc, &usData, 2);
  return usData;
}
...
{
  uint32_t ulData = 0;
  (void)ptDev->pfnHwIfRead(ptDev, pvSrc, &ulData, 4);
  return ulData;
}
