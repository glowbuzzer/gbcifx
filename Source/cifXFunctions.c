...
***************************************************************************************
  ...
  $Id: cifXFunctions.c 13761 2020-11-16 12:05:13Z AMinor $:
  ...
  Changes:
    ...
    2019-10-11  Use internal buffer for endianess conversion in xChannelControlBlock()
                Propagate prototype changes of endianess conversion function
...
int32_t APIENTRY xSysdeviceResetEx(CIFXHANDLE hSysdevice, uint32_t ulTimeout, uint32_t ulMode)
{
  ...
  {
    lRet = DEV_DoSystemStart(ptSysDevice, ulTimeout, (HIL_SYS_CONTROL_RESET_PARAM_MASK & ulMode));
  }
  else if(CIFX_RESETEX_BOOTSTART == (HIL_SYS_CONTROL_RESET_MODE_MASK & ulMode))
  {
    lRet = DEV_DoSystemBootstart(ptSysDevice, ulTimeout, (HIL_SYS_CONTROL_RESET_PARAM_MASK & ulMode));
  }
  else if(CIFX_RESETEX_UPDATESTART == (HIL_SYS_CONTROL_RESET_MODE_MASK & ulMode))
  {
    lRet = DEV_DoUpdateStart(ptSysDevice, ulTimeout, (HIL_SYS_CONTROL_RESET_PARAM_MASK & ulMode));
  }
  else
  {
  ...
  if( CIFX_NO_ERROR == lRet)
  {
    if( SUCCESS_HIL_OK == (lRet = LE32_TO_HOST(tDirListCnf.tHead.ulSta)) )
    {
      ...
      } else
      {
        ...
        int* piT = (int*)&ptDirectoryInfo->hList;
        *piT = 1;
                                     ...
                                     PFN_RECV_PKT_CALLBACK  pfnRecvPktCallback, void* pvUser)
{
  ...
  if(CIFX_NO_ERROR == lRet)
  {
    if( SUCCESS_HIL_OK == (lRet = (LE32_TO_HOST(tDirListCnf.tHead.ulSta))) )
    {
      ...
      {
        int* piListEntry = (int*)&ptDirectoryInfo->hList;
